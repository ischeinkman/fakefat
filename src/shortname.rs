...
impl fmt::Display for ShortName {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let name_len = self
            ...
            .rposition(|b| *b != b' ' && *b != 0)
            .unwrap_or(0);
        let name_part = if name_len > 0 {
            unsafe { from_utf8_unchecked(&self.name[..name_len]) }
        } else {
            "{NONE}"
        };
        let ext_len = self
            ...
            .rposition(|b| *b != b' ' && *b != 0)
            .unwrap_or(0);
        let ext_part = if ext_len > 0 {
            unsafe { from_utf8_unchecked(&self.ext[..ext_len]) }
        } else {
            "{NONE}"
        };
...
impl ShortName {
    ...
    pub fn convert_str<T: AsRef<str>>(name: T, duplicate_count: u8) -> ShortName {
        ...
        let mut name_part = to_valid_shortname(name_part_raw);
        while name_part.len() < 8 {
            name_part.push(' ');
        }
        let mut ext_part = to_valid_shortname(ext_part_raw);
        while ext_part.len() < 3 {
            ext_part.push(' ');
        }

        let mut retval = ShortName::default();
        (&mut retval.ext).copy_from_slice(&ext_part.as_bytes()[0..3]);
        (&mut retval.name).copy_from_slice(&name_part.as_bytes()[0..8]);
